...
header:
  title: "What To Do (And Not to Do) with Modeling Proportions/Fractional Outcomes"
  summary: "I discuss the available approaches in R for handling proportional/fractional outcomes, and present my new R package `ordbetareg` as a synthesis of the best of what is currently available."
  image: "headers/speed_limit.jpeg"
  date: "2022-04-02T15:00:00"
tags: ["R", "Proportions","Beta Regression"]
categories: ["R"]
---
...
The plot shows that most scores are equal to 1, indicating all seats are elected via popular vote. There are some legislatures, though, where everyone is appointed, and some where there are some who are appointed and elected. These countries include the following:
...
```{r pred_lm,fig.cap="OLS Predicted Values of Elected Lower Chamber Seats Given Power of Social Groups"}
...
One issue is that this prediction is close to the maximum of the scale: `r max(lm_pred$fit)`. While OLS doesn't predict out of bounds, the confidence interval of the prediction is likely to exceed zero. We can test that by getting the prediction error:

```{r max_ols}
max(lm_pred$fit + 2*lm_pred$se.fit)
```

We can see that high end of the 5% to 95% confidence interval reaches 1.09. In other words, OLS predicts that when social groups are moderately powerful, 109% of seats will be elected. This possibility, of course, is ridiculous, and makes no sense given our scale. Again, OLS can't respect bounds, which means that we can get intervals that can't be mapped on to the underlying scale in a meaningful sense.
...
```{r pred_frac,fig.cap="Fractional Logit Predicted Values of Elected Lower Chamber Seats Given Power of Social Groups"}
...
```{r pred_beta_reg,fig.cap="Beta Regression Predicted Values of Elected Lower Chamber Seats Given Power of Social Groups"}
...
```{r zoib_pred,fig.cap="OLS Predicted Values of Elected Lower Chamber Seats Given Power of Social Groups"}
...
Marginal effects are different than predictions because what we have is the effect of `power_groups` on `appoint_chamber` rather than the predicted level of `appoint_chamber`. The ultimate result shows that increasing levels of social group power is associated with greater numbers of appointed seats up until fairly high levels of power. At the highest levels, there does not seem to be much of a relationship. As a result, it would seem that the ZOIB model has a different spin than OLS, showing really strong associations at lower levels of social group power but little association at high levels of group power.

Of course, a big complication is that can't get a single effect from the model's coefficients, only by averaging over them and creating marginal effects. To address this issue, I next use ordered beta regression.

### Ordered Beta

The ordered beta regression model, which uses the `ordbetareg` package which is itself a front-end to `brms`, uses a simpler syntax because there is only one linear model. As such, the syntax is closer to `lm` or `glm`:
...
Although we know the relationship from the coefficients, we can also look at predicted values:w

```{r ord_beta_pred,fig.cap="OLS Predicted Values of Elected Lower Chamber Seats Given Power of Social Groups"}
...
```{r compare_coefs,fig.cap="Comparison of Ordered Beta and ZOIB Model Predictions of Elected Seats"}
...
```{r compare_coefs_all,fig.cap="Comparison of Model Predictions of Elected Seats to Ordered Beta Predictions (Gray Ribbon)"}
