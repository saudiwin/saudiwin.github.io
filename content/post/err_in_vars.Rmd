...
This tutorial is designed to present the indexes by the joint CoronaNet-Oxford CGRT policy projects. It shows how to download the estimated indices and also how to incorporate measurement error into analyses using the `eivtools` and `brms` R packages. 
...
```
The data is organized in a country-day-policy record format with the different policy indicators in columns. The fractional values indicate a proportion of the population that ha the following policy in place (i.e., at the provincial or municipal level). The high value for `primary_school` has to do with the fact that this variable naturally has 3 levels, from totally open to totally closed. For more information about the underlying indicators, you can see [our codebook](https://docs.google.com/document/d/1ScKWFv7jOHYVQlsbqvf--yU0mO2JscTsNEmb0SMXcsg/edit?usp=sharing).
...
Now we can use the `eivreg` as a function for estimating a linear regression model for our outcome, `retail`. We need to simplify our uncertainty measures because conventional measurement/errors-in-variables models can only handle one standard deviation/standard error per variable. As such, we will pass in the average posterior standard deviation as our estimate of `Sigma`, or the uncertainty of each index:
...
We can then see what our estimated associations are incorporating average standard errors for each index:
