...
Thankfully, this system has a single solution where $p_t$ is equal to \frac{393}{151268} (~0.0026) and $p_c$ is equal to \frac{655}{75634} (~0.0087). In other words, the proportion infected in the treatment group was about four times less than the control group. We'll assume that these distributions are the correct ones, and then sample $p_{ti}$ and $p_{ci}$ from Beta distributions that are fairly tight around these values. This will allow us to model treatment heterogeneity within the sample. The distribution of possible VEs given this heterogeneity can be seen in the plot below:
...
$$
VE_i \sim 1 - \frac{Beta(5000p_t,5000(1-p_t))}{Beta(5000p_c,5000(1-p_c))}
$$

Four subgroups are chosen to be roughly equal to the size of the half-dose group in the AstraZeneca study given a sample size of 11,636. I add a further step in that I assume that the probability of VE being reported for a subgroup $i$ ($Pr(r=1)$ is increasing in the rate of VE. As VE rises, the subgroup analysis is more likely to be reported. For these reasons, we can distinguish between the full range of VE estimates for subroup $i$ ($VE_i$) and the particular subgroup $r$ that is reported, $VE_{ir=1}$:

$$
Pr(VE_{ir}) = Pr(r|VE_i)VE_i + (1 - Pr(r|VE_i))VE_i
$$

This equation shows that the extent of the bias in using only reported results is equal to $(1 - Pr(r|VE_i))VE_i$, or the weighted probability that a result won't be reported conditional on the level of VE in the result.
...
Given this simulation, only a minority (`r round(mean(sim_data$VE_r),2)*100`%) of subgroup analyses are reported. The average VE for the reported subgroups is `r round(mean(sim_data$VE[sim_data$VE_r]),2)*100`% as opposed to a VE of `r round(mean(sim_data$VE),2)*100` across all simulations. As such, this adjustment makes it unlikely that subgroups with VEs with a low efficacy will end up being reported, while subgroups with higher VEs are more likely to be reported.

For each random draw, I can then fit a model that analyses only those analyses that are reported:

```{r code,message=F,warning=F,echo=F}
