...
categories: ["R", "Statistics" ,"Linear Models","Probability Theory"]
...
If you want another take on this, see this excellent StackOverflow post: <https://stats.stackexchange.com/questions/12262/what-if-residuals-are-normally-distributed-but-y-is-not/33320#33320>. It shows that, in fact, you can have Normally-distributed residuals *even when Y is not Normally-distributed* 🤯. So be careful with inferring Normality from residuals!
...
print(paste0("True SE of the Intercept: ", sd(over_sims_rando_beta$alpha)))
print(paste0("True SE of Beta: ", sd(over_sims_rando_beta$beta)))
...
se_pop <- sqrt(vcov(lmer_fit)["X","X"] +
               as.numeric(VarCorr(lmer_fit)$group["X","X"]))


print(paste0("lmer combined SE of Beta: ", se_pop))
print(paste0("OLS SE of Beta: ", ols_rand_slope_ses$coefficients[2,'Std. Error']))
print(paste0("True SE of Beta: ", sd(over_sims_rando_beta$beta)))

```

`lme4` estimate is a bit bigger than the true value due to some differences in how `lme4` estimates random slopes–I'll let the reader explore that one on their own 😁. It would also be possible to adjust the OLS standard errors with clustering around `group_id` if we want to continue using vanilla OLS.
