...
image: "../../files/img/headers/CDC_Fight_Flu_PSA.jpg"
categories: ["R","Panel Data", "COVID-19","Data Science"]
---
...
if(load_files) {
  ...
  cdc_masks <- read_csv("../../files/data/U.S._State_and_Territorial_Public_Mask_Mandates_From_April_10__2020_through_January_10__2021_by_County_by_Day.csv") %>% 
...
cdc_bars <- read_csv("../../files/data/U.S._State_and_Territorial_Orders_Closing_and_Reopening_Bars_Issued_from_March_11__2020_through_December_31__2020_by_County_by_Day.csv") %>% 
  mutate(date=date(mdy_hms(date)),
         FIPS_County=as.character(FIPS_County),
         FIPS_State=as.character(FIPS_State),
         FIPS_County=case_when(nchar(FIPS_County)==1~paste0("00",FIPS_County),
...
cdc_restaurants <- read_csv("../../files/data/U.S._State_and_Territorial_Orders_Closing_and_Reopening_Restaurants_Issued_from_March_11__2020_through_December_31__2020_by_County_by_Day.csv") %>% 
  mutate(date=date(mdy_hms(date)),
         FIPS_County=as.character(FIPS_County),
         FIPS_State=as.character(FIPS_State),
         FIPS_County=case_when(nchar(FIPS_County)==1~paste0("00",FIPS_County),
...
cdc_stay_home <- read_csv("../../files/data/U.S._State_and_Territorial_Stay-At-Home_Orders__March_15___December_31_by_County_by_Day.csv") %>% 
    mutate(date=date(mdy_hms(date)),
         FIPS_County=as.character(FIPS_County),
         FIPS_State=as.character(FIPS_State),
         FIPS_County=case_when(nchar(FIPS_County)==1~paste0("00",FIPS_County),
...
cdc_gather <- read_csv("../../files/data/U.S._State_and_Territorial_Gathering_Bans__March_11-December_31__2020_by_County_by_Day.csv") %>% 
  ...
  cdc_masks <- readRDS("../../files/data/cdc_masks.rds")
  cdc_gather <- readRDS("../../files/data/cdc_gather.rds")
  cdc_bars <- readRDS("../../files/data/cdc_bars.rds")
  cdc_restaurants <- readRDS("../../files/data/cdc_restaurants.rds")
  cdc_stay_home <- readRDS("../../files/data/cdc_stay_home.rds")
...
county_level_vote_share <- read_csv("../../files/data/countypres_2000-2016.csv") %>% 
...
nyt_data <- read_csv("../../files/data/us-counties.csv") %>% 
...
tests <- read_csv("../../files/data/states_daily_4pm_et.csv") %>% 
...
If you would like access to the code and data I used to write this blog post, please use this [Github repository](https://github.com/saudiwin/cdcmask).
...
all_coefs <- lapply(list(d1,d2,d3,d4,d5),function(d) slice(as_tibble(summary(d)$coeftable),1)) %>% 
  bind_rows %>% 
...
all_coefs_within <- lapply(list(d1_within,d2_within,d3_within,d4_within,d5_within),function(d) slice(as_tibble(summary(d)$coeftable),1)) %>% 
  bind_rows %>% 
  mutate(Model=c("1-20 Days",
                 "21-40 Days",
                 "41-60 Days",
...
all_coefs_between <- lapply(list(d1_between,d2_between,d3_between,d4_between,d5_between),function(d) slice(as_tibble(summary(d)$coeftable),1)) %>% 
  bind_rows %>% 
  mutate(Model=c("1-20 Days",
                 "21-40 Days",
                 "41-60 Days",
...
if(run_did) {
  ...
  att_gt(yname="cases_diff_log",
                          ...
                          cores=5,weightsname = NULL,
                          est_method="reg",
                          data=.)
  
  saveRDS(did_est,"../../files/data/did_est.rds")
  ...
  did_est <- readRDS("../../files/data/did_est.rds")
...
ag_did <- aggte(did_est,type="dynamic",na.rm=T)
ag_overall <- aggte(did_est,type="group",na.rm=T)
